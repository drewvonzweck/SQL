...
def teamPlayerCount(myConn, name):
    ...
    try:
        stmt = "SELECT count(p.playerid) FROM teams t, players p WHERE t.name = '"+name+"'  AND t.teamid = p.teamid ;"
...
# updateTeamCity(myConn, theTeamID, newTeamCity):
...
def updateTeamCity(myConn, theTeamID, newTeamCity):
    ...
    try:
        myCursor.execute("UPDATE Teams SET teamCity = %s WHERE teamID = %s",(newTeamCity,theTeamID))
...
def main():
    ...
    print("Count of players on team Birds is", teamPlayerCount(myConn,"Birds"))
    print("Count of players on team Skyscrapers is", teamPlayerCount(myConn,"Skyscrapers"))

    # Perform the calls to updateTeamCity described in Section 6 of Lab4, and print their outputs.
    # You have to write this yourselves.
    print("Number of teams with teamID 202 who moved to city Las Vegas is", updateTeamCity(myConn,202,"Las Vegas"))
    print("Number of teams with teamID 208 who moved to city Toronto is", updateTeamCity(myConn,208,"Toronto"))
    
    # Perform the calls to fireSomePlayers described in Section 6 of Lab4, and print their outputs.
    # You have to write this yourselves.
    print("Number of player fired in fireSomePlayers when maxFired equals 2 is", fireSomePlayers(myConn,2))
    print("Number of player fired in fireSomePlayers when maxFired equals 3 is", fireSomePlayers(myConn,3))
